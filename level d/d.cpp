...
void depth_first_search(ll vertex, bool visited[], vector<ll> *adl)
{

    visited[vertex] = 1;

    for (auto i : adl[vertex])
    {
        if (!visited[i])
        {
            depth_first_search(i, visited, adl);
        }
    }
}
    ...
    while (T--)
    {
        ...
        while (!vis[n - 1])
        {
            ...
            forn(i, n) if (vis[i])
            {
                if (i + 1 < n)
                {
                    depth_first_search(i + 1, vis2, adj);
                }
                if (i - 1 >= 0)
                {
                    depth_first_search(i - 1, vis2, adj);
